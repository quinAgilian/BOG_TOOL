...
# Burn record
BR=$(curl -s -X POST "$BASE/api/burn-record" \
  -H "Content-Type: application/json" \
  -d '{"deviceSerialNumber":"SN-BURN-AUTO","macAddress":"AA:BB:CC:DD:EE:FF","burnStartTime":"2025-02-25T10:00:00Z","burnDurationSeconds":45.5,"computerIdentity":"PC-TEST","deviceWrittenTimestamp":"2025-02-25T10:00:52Z","binFileName":"firmware_v1.0.5.bin","deviceWrittenSerialNumber":"SN-001-20250225","burnTestResult":"passed"}')
if echo "$BR" | grep -q '"ok":true'; then
  echo "  POST /api/burn-record: 通过"
else
  echo "  POST /api/burn-record: 失败 ($BR)"
fi
BR_LIST=$(curl -s "$BASE/api/burn-records?limit=5")
BR_N=$(echo "$BR_LIST" | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d.get('records',[])))" 2>/dev/null || echo "?")
echo "  GET /api/burn-records: 返回 $BR_N 条"
